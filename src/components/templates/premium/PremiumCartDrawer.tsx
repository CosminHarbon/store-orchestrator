...
export function PremiumCartDrawer({ commerce }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[80]">
      ...
      />
      <aside className="absolute right-0 top-0 h-full w-full max-w-md bg-[var(--prem-surface)] shadow-2xl flex flex-col animate-[premFadeUp_0.35s_ease]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--prem-line)]">
          <div className="flex items-center gap-2">
            <ShoppingBag className="h-5 w-5" />
            <h2 className="text-xl prem-display">{t('cart.yourBag')}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {!cart.length && (
            <div className="text-center py-16 space-y-3">
              <p className="text-[var(--prem-muted)]">{t('cart.empty')}</p>
              <button
                type="button"
          ...
          {recommended.length > 0 && cart.length > 0 && (
            <div className="pt-4 border-t border-[var(--prem-line)]">
        ...
        {cart.length > 0 && (
          <div className="border-t border-[var(--prem-line)] p-5 space-y-3 bg-[var(--prem-surface)]">
            <div className="flex justify-between text-sm">
              <span className="text-[var(--prem-muted)]">{t('summary.subtotal')}</span>
              <span className="font-semibold tabular-nums">{formatRon(cartSubtotal)}</span>
            </div>
            <p className="text-xs text-[var(--prem-muted)]">{t('cart.shippingAtCheckout')}</p>
            <button
              type="button"
              className="prem-btn prem-btn-primary w-full"
              onClick={() => {
                setCartOpen(false);
                setView('checkout');
