...
export function FloralProduct({ commerce }: Props) {
  const {
    apiKey,
    selectedProduct,
    setView,
  ...
  return (
    <div className="floral-container py-6 md:py-10 pb-28">
      <button
        type="button"
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-14">
        <div className="space-y-3">
          <button
            type="button"
            className="relative w-full aspect-[4/5] rounded-[var(--floral-radius)] overflow-hidden bg-[var(--floral-image-bg)] group"
          ...
          >
            ...
            {selectedProduct.has_discount && (
              <span className="absolute top-4 left-4 rounded-full bg-[var(--floral-sale)] text-white text-xs px-3 py-1">
                Sale
              </span>
            )}
          </button>
          {gallery.length > 1 && (
            <div className="flex items-center gap-2">
              <button
                type="button"
                className="p-2 rounded-full border border-[var(--floral-line)]"
                onClick={() => setImageIndex((i) => (i - 1 + gallery.length) % gallery.length)}
              >
                <ChevronLeft className="h-4 w-4" />
              </button>
              <div className="flex-1 floral-rail !gap-2">
                {gallery.map((src, i) => (
                  <button
                    key={`${src}-${i}`}
                    type="button"
                    className={`h-16 w-14 rounded-md overflow-hidden border-2 shrink-0 ${
                      i === imageIndex ? 'border-[var(--floral-ink)]' : 'border-transparent'
                    }`}
        ...
        <div className="space-y-5">
          ...
          {selectedProduct.description && (
            <p className="text-[var(--floral-muted)] leading-relaxed whitespace-pre-wrap">
              {selectedProduct.description}
            </p>
          )}
          ...
          <div className="flex items-start gap-3 text-sm bg-[var(--floral-surface)] border border-[var(--floral-line)] rounded-[var(--floral-radius)] p-4">
            ...
            <div>
              ...
              <p className="text-[var(--floral-muted)] mt-0.5">
          ...
          <div className="flex items-center gap-2 text-sm text-[var(--floral-muted)]">
          ...
          {customization?.show_reviews !== false && (
            <div className="pt-4 border-t border-[var(--floral-line)] space-y-4">
              <div className="flex items-end justify-between gap-3">
                <h3 className="text-xl floral-display">Reviews</h3>
                {productReviews.length > 0 && (
                  <p className="text-sm text-[var(--floral-muted)] tabular-nums">
                    {(
              ...
              {productReviews.length === 0 ? (
                <p className="text-sm text-[var(--floral-muted)]">No reviews yet — be the first.</p>
              ) : (
                <div className="space-y-3">
                  {productReviews.slice(0, 8).map((r) => (
                    <div key={r.id} className="text-sm space-y-1.5">
                      ...
                      {r.comment && <p className="text-[var(--floral-muted)]">{r.comment}</p>}
                      {r.merchant_reply && (
                        <div className="ml-2 pl-3 border-l border-[var(--floral-line)] text-[var(--floral-muted)]">
                          <span className="text-xs font-medium text-[var(--floral-fg)]">Store reply</span>
              ...
              <StorefrontReviewForm
                ...
                className="border-[var(--floral-line)] bg-[var(--floral-surface)]"
              />
            </div>
          )}
        </div>
      ...
      {related.length > 0 && (
        <section className="mt-16">
          <h2 className="text-3xl floral-display mb-6">Related products</h2>
      ...
      {recentProducts.filter((p) => p.id !== selectedProduct.id).length > 0 && (
        <section className="mt-12">
          <h2 className="text-3xl floral-display mb-6">Recently viewed</h2>
          <div className="floral-rail">
            {recentProducts
      ...
      {/* Sticky ATC */}
      <div className="fixed bottom-0 inset-x-0 z-40 border-t border-[var(--floral-line)] bg-[var(--floral-surface)]/95 backdrop-blur px-4 py-3 lg:hidden">
