...
export function useCursorAiStoreBuilder(opts?: { enabled?: boolean }) {
  ...
  // Poll get_run → complete_run while active (at most one in-flight complete_run).
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        if (session?.pipeline_status === 'ready' && !activeRun) return;
        ...
        if (session?.pipeline_status === 'ready' && (local === 'finished' || remote === 'FINISHED')) {
          setActiveRun(null);
        ...
        if ((remote === 'FINISHED' || local === 'finished') && !completing) {
          completing = true;
          try {
            ...
            // Keep completing=true after success so interval cannot re-ingest.
            if (!cancelled && activeRun) {
              /* leave lock until activeRun cleared */
            }
          }
        }
      } catch (e) {
