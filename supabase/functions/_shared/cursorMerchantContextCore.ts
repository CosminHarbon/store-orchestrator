/**
 * Deno copy of src/lib/ai-store-builder/merchantContext.ts (keep in sync).
 * Edge cannot import the Vite app tree.
 */
...
export function buildCursorStorefrontContext(input: MerchantContextInput): CursorStorefrontContext {
  ...
  const tagline = truncate(input.store.tagline || input.store.hero_subtitle || '', 160) || '';
