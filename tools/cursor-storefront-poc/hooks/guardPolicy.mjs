...
export const EDITABLE_PREFIXES = ['src/storefront/', 'src/components/', 'src/styles/', 'public/'];
...
const deny = (reason) => ({
  ...
  agent_message: `Blocked by SpeedVendors storefront policy: ${reason}. Only edit src/storefront/, src/components/, src/styles/ and public/, and only run the storefront build ("npm run build") or read-only inspection commands inside the workspace.`,
});
